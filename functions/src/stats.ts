 ...
 */
export async function updateRemainingStories(uid: string, firestore_db: firestore.Firestore) {
  ...
  if (userData.remainingStories <= 0) {
    logger.error(`User ${uid} tried to create a new story despite having no more remaining stories today.`);
    throw new HttpsError(
      "resource-exhausted",
